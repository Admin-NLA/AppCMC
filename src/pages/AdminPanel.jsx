...
const SECCIONES = [
  ...
  },
  {
    ...
    tabs: [
      ...
      { id: "control_funciones", label: "🎛 Control de Funciones", adminOnly: true },
      { id: "configuracion", label: "⚙️ Configuración", adminOnly: true },
      { id: "staff_panel", label: "📊 Panel Estadístico (Staff Panel)", adminOnly: false, external: "/staff" },
    ],
  },
];
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    if (activeTab === "control_funciones") loadControlFunciones();
    ...
    <div className="max-w-7xl mx-auto space-y-0">
      ...
      <div className="flex gap-5 min-h-[calc(100vh-200px)]">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* ─── CONTROL DE FUNCIONES ─── */}
          {activeTab === "control_funciones" && isAdmin && (
            <ControlFunciones config={controlFunciones} onSave={saveControlFunciones} flash={flash} />
          )}
          ...
          {/* Panel Estadístico ahora navega directo a /staff (Staff Panel real) — ver tab.external */}
...
// ════════════════════════════════════════════════════════════
// ── Helper: formatear fechas DATE de PostgreSQL ──────────────
const formatDate = (d, opts = { day: "numeric", month: "short", year: "numeric" }) => {
  ...
  try {
    ...
    const dt = new Date(s);
    return isNaN(dt.getTime()) ? "—" : dt.toLocaleDateString("es", opts);
  } catch { return "—"; }
};
...
function MapaStandsAdmin({ expositores, sedeActiva, flash, loadExpositores }) {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 rounded-2xl p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {sinPosicion.map(expo => {
              const cat = categorias.find(c => c.key === (expo.categoria || "").toLowerCase()); return (
      ...
      {/* Banner modo ubicar */}
      {standAColocar && (() => {
        const cat = categorias.find(ct => ct.key === (standAColocar.categoria || "").toLowerCase()); const canPlace = preview?.canPlace ?? true; return (
                  ...
                  <div className="flex flex-wrap items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-3 text-xs text-gray-500 mb-2">
                        ...
                        {ev.fecha_inicio && <span>🗓 {formatDate(ev.fecha_inicio, { day: "numeric", month: "short" })} – {ev.fecha_fin ? formatDate(ev.fecha_fin) : "?"}</span>}
...
function SedesCalendario({ sedes, reload, flash }) {
  ...
  const handleEliminarSede = async (sede, edicion) => {
    if (!confirm(`¿Eliminar ${sede} ${edicion} del calendario?`)) return;
    try {
      await API.delete(`/eventos/sedes-calendario/${sede}/${edicion}`);
      flash(`✅ ${sede} ${edicion} eliminado`); reload();
    } catch (err) { flash(err.response?.data?.error || "Error al eliminar", true); }
  };
                    ...
                    {isEdit ? (
                      ...
                      <div className="flex flex-wrap items-center gap-4">
                        ...
                        <div className="flex-1 grid grid-cols-2 md:grid-cols-3 gap-2 text-xs text-gray-500">
                          <span>📅 Inicio: <strong>{formatDate(item.fecha_inicio)}</strong></span>
                          <span>🏁 Fin: <strong>{formatDate(item.fecha_fin)}</strong></span>
                        ...
                        <button onClick={() => handleEliminarSede(item.sede, item.edicion)}
...
// ════════════════════════════════════════════════════════════
// CONTROL DE FUNCIONES POR ROL (Super Admin)
// ════════════════════════════════════════════════════════════
...
const ROLES_DISPLAY = [
  { id: "asistente_general", label: "A. General", color: "#6366f1" },
  { id: "asistente_curso", label: "A. Curso", color: "#8b5cf6" },
  { id: "asistente_sesiones", label: "A. Sesiones", color: "#7c3aed" },
  { id: "asistente_combo", label: "A. Combo", color: "#a855f7" },
  { id: "expositor", label: "Expositor", color: "#f59e0b" },
  { id: "speaker", label: "Speaker", color: "#10b981" },
  { id: "staff", label: "Staff", color: "#3b82f6" },
];

function ControlFunciones({ config, onSave, flash }) {
  const [local, setLocal] = useState({});
  const [saving, setSaving] = useState(false);

  useEffect(() => { setLocal(config || {}); }, [config]);

  const toggle = (funcKey, rol) => {
    setLocal(prev => {
      const curr = prev[funcKey] || {};
      return { ...prev, [funcKey]: { ...curr, [rol]: !curr[rol] } };
    });
  };

  const isActive = (funcKey, rol) => {
    if (local[funcKey] === undefined) {
      // Default: activo si el rol está en la lista de roles permitidos de la función
      const func = FUNCIONES_APP.find(f => f.key === funcKey);
      return func?.roles.includes(rol) ?? false;
    }
    return local[funcKey]?.[rol] ?? false;
  };

  const resetDefaults = () => {
    const defaults = {};
    FUNCIONES_APP.forEach(f => {
      defaults[f.key] = {};
      f.roles.forEach(r => { defaults[f.key][r] = true; });
    });
    setLocal(defaults);
  };
  ...
  const secciones = [...new Set(FUNCIONES_APP.map(f => f.seccion))];

  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold dark:text-white">🎛 Control de Funciones</h2>
          <p className="text-sm text-gray-500 mt-0.5">Define qué funciones de la app están activas para cada perfil de usuario.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={resetDefaults} className="flex items-center gap-2 border border-gray-300 text-gray-600 px-4 py-2 rounded-xl text-sm hover:bg-gray-50 transition">
            <RefreshCw size={14} /> Restaurar defaults
          </button>
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-semibold disabled:opacity-50 hover:bg-blue-700 transition">
      ...
      <div className="flex flex-wrap gap-2">
        {ROLES_DISPLAY.map(r => (
          <span key={r.id} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold text-white" style={{ backgroundColor: r.color }}>
            {r.label}
          </span>
        ))}
      </div>
      ...
      {secciones.map(seccion => {
        const funcs = FUNCIONES_APP.filter(f => f.seccion === seccion);
        return (
          <div key={seccion} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-700">
              <p className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">{seccion}</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-gray-700">
                    <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase min-w-[200px]">Función</th>
                    {ROLES_DISPLAY.map(r => (
                      <th key={r.id} className="px-3 py-2 text-center text-xs font-semibold whitespace-nowrap" style={{ color: r.color }}>
                        {r.label}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {funcs.map(func => (
                    <tr key={func.key} className="hover:bg-gray-50 dark:hover:bg-gray-700/40 transition">
                      <td className="px-4 py-2.5">
                        <p className="text-sm font-medium text-gray-900 dark:text-white">{func.label}</p>
                        <p className="text-xs text-gray-400 font-mono">{func.key}</p>
                      </td>
                      {ROLES_DISPLAY.map(r => {
                        const applicable = func.roles.includes(r.id);
